...
import com.hao.common.adapter.BaseRecyclerViewHolder;
 ...
 * 描述:拖拽排序九宫格图片控件
 */
public class BGASortableNinePhotoLayout extends RecyclerView implements OnItemChildClickListener, OnRVItemClickListener {
    ...
    private void initDefaultAttrs() {
        ...
        mSortable = true;
        mEditable = true;
        mDeleteDrawableResId = R.mipmap.pp_ic_delete;
        mDeleteDrawableOverlapQuarter = false;
        mMaxItemCount = 9;
        mItemSpanCount = 3;
        mItemWidth = 0;
        mItemCornerRadius = 0;
        mPlusDrawableResId = R.mipmap.pp_ic_plus;
        mItemWhiteSpacing = UIUtil.dp2px(4);
        mPlaceholderDrawableResId = R.mipmap.pp_ic_holder_light;
        mOtherWhiteSpacing = UIUtil.dp2px(100);
    }
    ...
    private void afterInitDefaultAndCustomAttrs() {
        if (mItemWidth == 0) {
            mItemWidth = (ViewUtils.getScreenWidth(getContext()) - mOtherWhiteSpacing) / mItemSpanCount;
        } else {
    ...
    private void calculatePhotoTopRightMargin() {
        if (mDeleteDrawableOverlapQuarter) {
            ...
            int deleteDrawablePadding = getResources().getDimensionPixelOffset(R.dimen.pp_size_delete_padding);
    ...
    private class PhotoAdapter extends BaseRecyclerViewAdapter<String> {
        ...
        public PhotoAdapter(RecyclerView recyclerView) {
            super(recyclerView, R.layout.pp_item_nine_photo);
            mImageSize = ViewUtils.getScreenWidth(mContext) / (mItemSpanCount > 3 ? 8 : 6);
        }
        ...
        protected void fillData(BaseViewHolderHelper helper, int position, String model) {
            ...
            if (isPlusItem(position)) {
                ...
                ImageLoader.getInstance().displayImage(mContext,model,helper.getImageView(R.id.iv_item_nine_photo_photo),mImageSize,mImageSize);
            }
        }
    }
    ...
    private class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public void onSelectedChanged(ViewHolder viewHolder, int actionState) {
            if (actionState != ACTION_STATE_IDLE) {
                ...
                ((BaseRecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo_photo).setColorFilter(getResources().getColor(R.color.pp_photo_selected_mask));
            }
        ...
        public void clearView(RecyclerView recyclerView, ViewHolder viewHolder) {
            ...
            ((BaseRecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo_photo).setColorFilter(null);
